...
/**
 * @class lfd
 * Manages lightweight file descriptors with a configurable buffer system and operation flags.
 * Supports both circular and non-circular buffer modes, as well as signaling for read and write notifications.
 */
struct  lfd
{
    CLASSNAME(lfd)
public:
    ...
    /**
     * @struct flags
     * Represents a set of state flags associated with the `lfd` class for managing its activity and lifecycle.
     *
     * The flags control key behaviors such as whether the `lfd` is actively monitored, paused, or marked for removal.
     *
     * @details
     * - `active`: Indicates if the `lfd` is active and being monitored. If inactive, it is not monitored.
     * - `del`: Indicates that the `lfd` is inactive and set to be removed from the epoll set, but not flagged for complete termination ("killing").
     * - `pause`: Denotes the `lfd` is in a paused state, halting its polling activities until resumed by setting the `active` flag.
     * - `kill`: Marks the `lfd` for termination and removal from both the epoll set and the listener’s list.
     */
    struct flags
    {
