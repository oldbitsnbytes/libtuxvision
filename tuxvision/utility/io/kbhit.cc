...
///
rem::code kbhit::parse(lfd &_fd)
{
    auto * b = _fd.tail();
    //auto l = sys::debug(); l << "buffer size:" << color::lime << _fd.size() << color::r << "..." << l;
    u64 code = *b;
    kbhit kb{};
    if(_fd.size() == 1){
    ...
    if(code==0x1b) // CSI start
    {
        ...
        do{
            ...
            {
                ////sys::debug() << "query code :" << color::yellow << std::format("0x{:016X}", code) << color::z << //sys::eol;
                _fd.sync_tail(b);
                console::push_event({.type = console::event::evt::KEV, .data={.kev=kb}});
                return rem::code::ready;
            }
