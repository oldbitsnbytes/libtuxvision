...
lfd::~lfd()
{
    ////sys::debug() << "id:'" << color::yellow << _id << color::z << "' " << //sys::eol;
...
lfd& lfd::operator<<(u8 u)
{
    if (full())
    {
        //sys::error() << "Attempt to write into a full lfd";
        return *this;
    }
    *_head++ = u;
    return *this;
...
lfd& lfd::operator>>(u8& u)
{
    ...
    {
        u = 0;
        auto l = sys::warning(); l << "buffer is empty." << l;
        return *this;
    }
    u = *_tail++;
    return *this;
...
rem::code lfd::init()
{
    ...
    if(!(_bits & lfd::EXT))
    {    delete [] _buffer_ptr;
        //sys::info() << " Allocating " << color::yellow << _window_block_size << color::z << " bytes to lfd:"  << _id <<  //sys::eol;
    ...
    //sys::info() <<  color::yellow << free() << color::z << " bytes free." << //sys::eol;
...
rem::action lfd::_read()
{
    if(full())
    {
        //sys::warning() << " buffer is full" << //sys::endl;
        pause();
    ...
    ////sys::debug() << " number of bytes to read in #" << color::yellow << _fd << color::reset << ":" << color::lime << _waiting_bytes << //sys::endl;
    if(_waiting_bytes < 0)
    {
        //sys::error() << "decriptor #" << color::red4 << _fd << color::reset << " :" << color::red << std::strerror(errno) << color::z << //sys::endl;
        return rem::action::end;
    }
    ////sys::info() <<  color::yellow << free() << color::z << " bytes free." << //sys::eol;
    const auto nbytes = std::min((size_t)_waiting_bytes,free());
    ////sys::write() << " effective number of bytes to read in #" << color::yellow << _fd << color::reset << ":" << color::lime << nbytes << //sys::endl;

    if(!nbytes)
    {

        //sys::message() << " triggering descriptor on zero-byte signal..." << //sys::eol;
        //...
    ...
    //     auto l = sys::debug(1); l << color::yellow << nbytes << color::r << " bytes: {" << color::lighcoreateblue << s << color::r << "}" << l;
    // }
